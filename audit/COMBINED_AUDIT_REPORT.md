...
**Date**: 2026-08-08
**Auditors**: Antigravity (AI Code Auditor)
**Scope**: Full repository review — architecture, correctness, mathematical soundness, performance, code quality, testing, packaging
**Commit**: `5c76919` (Initial release)

> [!NOTE]
> This report is a **non-destructive merge** of two independent audits:
>
> - **Audit A** — [GENERAL_AUDIT_REPORT.md](file:///home/feildaw/diffpfa/audit/GENERAL_AUDIT_REPORT.md): Full-scope review covering architecture, correctness, code quality, testing, and packaging.
> - **Audit B** — [correctness_audit_report.md](file:///home/feildaw/diffpfa/audit/correctness_audit_report.md): Focused review targeting mathematical correctness (primary), performance (secondary), and bugs/gotchas (tertiary).
>
> Each finding is tagged with its provenance: `[A]`, `[B]`, or `[A+B]` (corroborated by both).
> Original report text is preserved; where both audits cover the same topic, both perspectives are presented.
...
### Audit B — Correctness
The core mathematical engine is structurally sound and effectively implements PyTorch-based differentiable PFA. However, the evaluation identified a fundamental spatial-variance bug in the CZT implementation, performance bottlenecks due to improper GPU cache handling, and misconceptions in the TODO items regarding NUFFT oversampling.

### Combined Severity Breakdown

| Severity | Count | Description |
|----------|-------|-------------|
| 🔴 Critical | 5 | Bugs or design flaws that cause incorrect behavior or runtime failures |
| 🟠 Major | 9 | Significant code quality, correctness, performance, or maintainability issues |
| 🟡 Minor | 7 | Style, packaging, and polish items |
...
### 3.1 `[A]` Polarization Extraction Broken in BOTH Backends
...
**Sarpy backend** ([sarpy_cphd.py](file:///home/feildaw/diffpfa/diffpfa/io/sarpy_cphd.py) lines 109-110):
```python
...
**Sarkit backend** ([sarkit_cphd.py](file:///home/feildaw/diffpfa/diffpfa/io/sarkit_cphd.py) lines 121-124):
```python
...
### 3.2 `[A]` Abstraction Leak: PFAEngine Reaches Through Reader to Raw Backend

**Files**: [pfa_engine.py](file:///home/feildaw/diffpfa/diffpfa/algo/pfa_engine.py) — Lines 163, 345, 531
...
#### Remediation Plan
Add Reference Geometry fields to `CPHDMetadata` in [base.py](file:///home/feildaw/diffpfa/diffpfa/io/base.py):
```python
@dataclass
class CPHDMetadata:
    # ... existing fields ...
...
### 3.3 `[A]` Unconditional Sarpy Import in `io/__init__.py` Breaks Sarkit-Only Environments

**File**: [diffpfa/io/\_\_init\_\_.py](file:///home/feildaw/diffpfa/diffpfa/io/__init__.py) — Lines 8-9
...
### 3.4 `[A]` CLI Hardcodes Sarpy Backend, Bypasses Factory System

**File**: [cli.py](file:///home/feildaw/diffpfa/diffpfa/cli.py) — Lines 6, 27-28
...
### 3.5 `[B]` The `ku_center` Error — CZT Spatial Variance Bug
...
### 4.1 `[A]` Massive Code Duplication Across Three Processing Modes

**File**: [pfa_engine.py](file:///home/feildaw/diffpfa/diffpfa/algo/pfa_engine.py)
...
### 4.2 `[A]` Hybrid Mode: Incomplete Deconvolution (Range Dimension Missing)

**File**: [pfa_engine.py](file:///home/feildaw/diffpfa/diffpfa/algo/pfa_engine.py) — Lines 476-482
...
### 4.3 `[A]` `scipy.fft.next_fast_len` Imported Inside Hot Loops

**Files**: [pfa_engine.py](file:///home/feildaw/diffpfa/diffpfa/algo/pfa_engine.py) Lines 218, 399; [nufft_torch.py](file:///home/feildaw/diffpfa/diffpfa/algo/nufft_torch.py) Lines 45, 96, 200
...
### 4.4 `[A]` Sarkit Backend Missing ImageArea/ExtendedArea Parsing

**File**: [sarkit_cphd.py](file:///home/feildaw/diffpfa/diffpfa/io/sarkit_cphd.py) — Lines 53-54
...
### 4.5 `[A+B]` NUFFT Scatter Operations — Differentiability and Performance

Both audits independently identified the NUFFT gridding operations as problematic, from complementary angles.

#### Audit A — Differentiability Concern

**File**: [nufft_torch.py](file:///home/feildaw/diffpfa/diffpfa/algo/nufft_torch.py) — Lines 252-256
...
#### Audit B — Performance Concern

The PyTorch native NUFFT loops in `nufft_torch.py` (specifically `nufft_2d_type1_torch`) rely on standard Python `for` loops and `grid.index_put_()` with `accumulate=True` over sliding windows (`J x J`). This is severely bottlenecked by PyTorch's CPU overhead for small dispatch operations.

**Remediation (Audit B):**
While `torch.compile` helps, creating a custom Triton kernel for the 2D non-uniform scattering would yield a 10x-50x speedup for the NUFFT mode.

---

### 4.6 `[A]` `torch.compile` Mutates Global State

**File**: [pfa_engine.py](file:///home/feildaw/diffpfa/diffpfa/algo/pfa_engine.py) — Lines 59-65
...
### 4.7 `[A]` `czt_resample_kspace_1d` Parameter Name Mismatch

**File**: [czt_torch.py](file:///home/feildaw/diffpfa/diffpfa/algo/czt_torch.py) — Line 126
...
### 4.8 `[B]` Destructive GPU Memory Management

**Issue:** In [pfa_engine.py](file:///home/feildaw/diffpfa/diffpfa/algo/pfa_engine.py) (lines 253-255, 288-289, 434-435, 442-444, 464-465), there are repeated calls to `torch.cuda.empty_cache()` inside the core tight processing loops.
```python
del sig_patch
del Kr
...
#### Remediation Plan
Remove all `torch.cuda.empty_cache()` calls within the processing loops. `empty_cache()` forces full GPU synchronization and flushes the PyTorch memory allocator cache, making subsequent tensor allocations extremely slow. Merely deleting the tensors (`del sig_patch`) is sufficient for PyTorch's caching allocator to reuse the VRAM immediately for the next batch.

---

### 4.9 `[B]` NUFFT Oversampling Misunderstanding in README TODOs
...
**Correction:** This is a misunderstanding of NUFFT mathematics. The 1.5 `oversample` factor determines the size of the *intermediate* padded K-space grid (`M_u`, `M_r`) prior to the IFFT, which prevents circular convolution aliasing caused by the Kaiser-Bessel kernel's roll-off. It does not dictate the output image size (`grid_size_u`, `grid_size_r`). The `oversample` factor should remain (at least 1.5x to 2x).
...
> [!IMPORTANT]
> Audit A (§5.6) also flagged the hardcoded `1.5` oversample factor but recommended making it configurable. Audit B clarifies that the value itself is correct for NUFFT — but making it configurable via `PFAConfig` is still reasonable for advanced users.

---

## 5. Minor Issues (🟡)

### 5.1 `[A]` No `setup.py`, `pyproject.toml`, or `requirements.txt`
...
### 5.2 `[A]` `conftest.py` Uses `sys.path` Hacking

**File**: [conftest.py](file:///home/feildaw/diffpfa/conftest.py)
...
### 5.3 `[A]` `visualize_sicd.py` Only Supports Sarpy

**File**: [visualize_sicd.py](file:///home/feildaw/diffpfa/visualize_sicd.py) — Line 4
...
### 5.4 `[A]` `test_outputs/` Committed to Git with ~1.7GB of NITF Data
...
### 5.5 `[A]` Vim Swap File Committed

**File**: `tests/.test_pfa.py.swp` (16KB) — an editor artifact that should be gitignored.

---

### 5.6 `[B]` Size Discrepancy between sarpy and sarkit SICDs

**Issue:** A TODO mentions size discrepancies between `sarkit` and `sarpy` SICD outputs.

**Correction:** Both engines write exact `RE32F_IM32F` (complex64) pixel types with matching dimensions. Discrepancies in byte footprint arise because NITF files group data into blocked segments (e.g., 512x512 sub-blocks) and append varying degrees of XML metadata strings. `sarkit` and `sarpy` utilize different default XML layout spacing and different default block sizing strategies. This is expected behavior and not an algorithmic bug.

---

### 5.7 `[B]` Complex Arithmetic Promotion Warning

**Issue:** In [channel_combine.py](file:///home/feildaw/diffpfa/diffpfa/algo/channel_combine.py): `curr_aligned = curr_img * torch.exp(1j * delta_phi)`.

**Correction:** While recent PyTorch handles the implicit conversion of native Python `complex` (`1j`) against a PyTorch tensor, to ensure strict type promotion (especially under `torch.compile`), it is safer to write: `torch.exp(1j * delta_phi.to(torch.complex64))` or similar native torch complex constructors.
...
Both audits identified critical issues with the test suite. Audit A performed a comprehensive survey; Audit B specifically called out the `test_mem.py` global-scope crash.

| File | Type | Assertions | Data Dependency | CI-Safe |
|------|------|-----------|----------------| --------|
| [test_pfa.py](file:///home/feildaw/diffpfa/tests/test_pfa.py) | Pytest | ✅ (2 unit, 4 integration) | Hardcoded path | ⚠️ Partial |
| [test_grid.py](file:///home/feildaw/diffpfa/tests/test_grid.py) | Script | ❌ None | Hardcoded path | ❌ Crashes |
| [test_mem.py](file:///home/feildaw/diffpfa/tests/test_mem.py) | Script | ❌ None | Hardcoded path + CUDA | ❌ Crashes |
| [test_metadata.py](file:///home/feildaw/diffpfa/tests/test_metadata.py) | Script | ❌ None | Hardcoded path | ❌ Crashes |
| [test_txpos.py](file:///home/feildaw/diffpfa/tests/test_txpos.py) | Script | ❌ None | Hardcoded path | ❌ Crashes |
| [test_err.py](file:///home/feildaw/diffpfa/tests/test_err.py) | Script | ❌ None | None | ⚠️ No-op |
| [test_lxml.py](file:///home/feildaw/diffpfa/tests/test_lxml.py) | Script | ❌ None | None | ⚠️ No-op |
| [test_sarkit.py](file:///home/feildaw/diffpfa/tests/test_sarkit.py) | Script | ❌ None | None | ⚠️ No-op |
| [compare_modes.py](file:///home/feildaw/diffpfa/tests/compare_modes.py) | CLI tool | ❌ None | CLI arg | ❌ Manual |

### Critical Anti-Patterns

1. **Silent skip masquerading as pass** — `test_pfa.py` uses `return` instead of `pytest.skip()` when data is missing. CI reports 6/6 tests PASSED when only 2 actually ran. `[A]`
2. **Top-level execution in test files** — `test_grid.py`, `test_mem.py`, `test_metadata.py`, `test_txpos.py` execute code at module import time. Pytest test discovery will crash if the CPHD file is missing. `[A+B]`
3. **No synthetic test data** — Every integration test requires a proprietary 1GB+ CPHD file at a hardcoded absolute path. `[A]`
4. **Hardcoded `device="cuda"`** — Integration tests in `test_pfa.py` fail on CPU-only machines. `[A]`
...
5. Wrap `test_mem.py` global-scope execution inside proper pytest functions `[B]`
...
> [!WARNING]
> The NUFFT gridding operations are the weakest link for differentiability. While they will produce *a* gradient via autograd, the gradient may be **incorrect or non-deterministic on GPU** due to race conditions in scatter-accumulate operations. This should be validated with a finite-difference gradient check before relying on it for autofocus workflows.
...
|----------|-------|--------|--------|--------|
| **P0** | §3.1 — Polarization broken in both backends | `[A]` | 1 hr | Metadata correctness |
| **P0** | §3.2 — Slant plane abstraction leak (crashes with sarkit) | `[A]` | 2-3 hrs | Correctness |
| **P0** | §3.3 — Unconditional sarpy import (crashes without sarpy) | `[A]` | 30 min | Usability |
| **P0** | §3.4 — CLI hardcodes sarpy backend | `[A]` | 15 min | Usability |
| **P0** | §3.5 — CZT `ku_center` spatial variance bug | `[B]` | 3-4 hrs | Mathematical correctness |
| **P1** | §4.4 — Sarkit missing ImageArea parsing | `[A]` | 2-3 hrs | Correctness |
| **P1** | §4.1 — Extract duplicated code into shared methods | `[A]` | 3-4 hrs | Maintainability |
| **P1** | §4.8 — Remove `empty_cache()` from processing loops | `[B]` | 30 min | Performance |
| **P1** | §6 Phase 1 — Fix test anti-patterns | `[A+B]` | 1-2 hrs | CI reliability |
| **P2** | §4.5 — NUFFT scatter differentiability + performance | `[A+B]` | 4-6 hrs | Correctness (ML) + Perf |
| **P2** | §4.6 — torch.compile global mutation | `[A]` | 1 hr | Correctness |
| **P2** | §4.9 — Clarify NUFFT oversample semantics | `[B]` | 1 hr | Documentation |
| **P2** | §6 Phase 2-3 — Test reorg + synthetic fixtures | `[A]` | 2-3 days | Test coverage |
| **P3** | §5.1 — Add pyproject.toml | `[A]` | 1-2 hrs | Packaging |
| **P3** | §5.4 — Remove tracked NITF binaries | `[A]` | 30 min | Repo hygiene |
| **P3** | §4.3 — Move imports out of loops | `[A]` | 15 min | Code quality |
| **P3** | §4.7 — Rename `L_r` parameter | `[A]` | 15 min | Readability |
| **P3** | §5.5 — Remove swap file | `[A]` | 5 min | Repo hygiene |
| **P3** | §5.6 — SICD size discrepancy is expected (update docs) | `[B]` | 15 min | Documentation |
| **P3** | §5.7 — Complex arithmetic type safety | `[B]` | 15 min | Robustness |
...
8. **Core mathematical engine is structurally sound** — The overall PFA pipeline effectively implements differentiable SAR image formation in PyTorch.

---

## Appendix: Source Report Index

| Section in Combined Report | Audit A Section | Audit B Section |
|---|---|---|
| §3.1 Polarization | §3.1 | — |
| §3.2 Abstraction Leak | §3.2 | — |
| §3.3 Unconditional Import | §3.3 | — |
| §3.4 CLI Hardcodes Sarpy | §3.4 | — |
| §3.5 `ku_center` Error | — | §1.1 |
| §4.1 Code Duplication | §4.1 | — |
| §4.2 Hybrid Deconvolution | §4.2 | — |
| §4.3 Import in Loops | §4.3 | — |
| §4.4 Sarkit ImageArea | §4.4 | — |
| §4.5 NUFFT Scatter | §4.5 | §2.2 |
| §4.6 torch.compile Global | §4.6 | — |
| §4.7 `L_r` Naming | §4.7 | — |
| §4.8 GPU Cache | — | §2.1 |
| §4.9 NUFFT Oversample | §5.6 | §1.2 |
| §5.1 No Packaging | §5.1 | — |
| §5.2 sys.path Hacking | §5.2 | — |
| §5.3 Visualize Sarpy-Only | §5.3 | — |
| §5.4 NITF in Git | §5.4 | — |
| §5.5 Swap File | §5.5 | — |
| §5.6 SICD Size Discrepancy | — | §3.2 |
| §5.7 Complex Promotion | — | §3.3 |
| §6 Test Suite | §6 | §3.1 |
| §7 Differentiability | §7 | — |
