...
- `compare_modes.py`: End-to-end integration test that runs the CPHD through `nufft` and `cztnufft` modes, saves them to SICD-U using `sarkit`, and generates phase difference plots.
- `test_grid.py`: Validates Cartesian mapping equations.
- `test_mem.py`: Audits VRAM utilization and peak tracking.
- `test_metadata.py`: Validates generic CPHD metadata extraction.
